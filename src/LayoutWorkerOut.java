...
public class LayoutWorkerOut extends JFrame {
    ...
    private void verifyAndRemoveUser(String login) {
        ...
        if (conn != null) {
            try {
                ...
                // Najpierw pobierz ID i nazwisko mechanika
                ...
                if (mechanicId != null && providedSurname.equals(actualSurname)) {
                    // Dodanie wpisu do tabeli zwolnienia
                    ...
                    this.dispose();
                    // Tutaj wróć do poprzedniego layoutu
                } else {
                    conn.rollback();
                ...
                try {
                    if (conn != null && !conn.isClosed()) {
                        conn.setAutoCommit(true); // Przywrócenie domyślnego trybu autocommit
                        conn.close();
                    }
                } catch (SQLException ex) {
                    ex.printStackTrace();
    ...
    // Klasa DatabaseConnection z metodą Connector() z klasy LayoutChange
    static class DatabaseConnection {
        public static Connection Connector() {
            // Tutaj należy podać dane swojej bazy danych
