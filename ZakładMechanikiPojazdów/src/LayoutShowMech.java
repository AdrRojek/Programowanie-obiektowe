...
public class LayoutShowMech extends JFrame{
    ...
    public LayoutShowMech(){
        ...
        zmieńButton.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                String mechanicId = textField1.getText(); // Pobieranie ID mechanika z textField1
                if (!mechanicId.isEmpty()) {
                    LayoutShowMech.this.dispose(); // Zamknięcie obecnego okna
                    new LayoutChange(mechanicId).setVisible(true); // Otwarcie nowego okna z przekazaniem ID
                } else {
